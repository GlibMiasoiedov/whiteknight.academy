...
<div class="tutor-checkout-details cp-summary">

	<?php do_action( 'tutor_before_checkout_order_details', $course_list ); ?>

	<div class="tutor-checkout-details-inner cp-summary__card">
		<div class="cp-summary__header">
        <h5 class="cp-summary__title">
           <?php esc_html_e( 'Your order', 'tutor' ); ?>
        </h5>
    ...
    <div class="cp-summary__total">
				...
				if ( $plan_info ) {
					...
					if ( is_array( $course_list ) && count( $course_list ) ) :
						...
						foreach ( $checkout_data->items as $item ) :
							...
							?>
							<div class="tutor-checkout-course-item cp-item" data-course-id="<?php echo esc_attr( $item->item_id ); ?>">
		...
		<div class="tutor-checkout-detail-item tutor-checkout-summary">
			...
			<?php if ( $show_coupon_box ) : ?>
                <div class="cp-coupon">
                  <div class="cp-coupon__label">
                     <?php esc_html_e( 'Promo code', 'tutor' ); ?>
                  </div>
                  <div class="tutor-apply-coupon-form cp-coupon__row">
                     <input
                         type="text"
                         name="coupon_code"
                         class="cp-coupon__input"
                     ...
                     <button
                         type="button"
                         id="tutor-apply-coupon-button"
                         class="tutor-btn tutor-btn-secondary cp-coupon__btn"
                         data-object-ids="<?php echo esc_attr( implode( ',', $object_ids ) ); ?>"
                     >
