...
if ( function_exists( 'rook_elementor_is_preview' ) && rook_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			rook_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}
	...
	rook_new_main_query(
		array(
			'post_type'      => rook_get_theme_option( 'post_type' ),
			'category'       => rook_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'rook_filter_get_template_part', rook_blog_archive_get_template() ) );
}
