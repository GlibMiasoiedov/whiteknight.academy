...
if ( ! function_exists( 'rook_storage_get' ) ) {
	/**
	 ...
	 */
	function rook_storage_get( $var_name, $default = '' ) {
		global $ROOK_STORAGE;
		return isset( $ROOK_STORAGE[ $var_name ] ) ? $ROOK_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'rook_storage_set' ) ) {
	/**
	 ...
	 */
	function rook_storage_set( $var_name, $value ) {
		global $ROOK_STORAGE;
		$ROOK_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'rook_storage_empty' ) ) {
	/**
	 ...
	 */
	function rook_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $ROOK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $ROOK_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $ROOK_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'rook_storage_isset' ) ) {
	/**
	 ...
	 */
	function rook_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $ROOK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $ROOK_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $ROOK_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'rook_storage_unset' ) ) {
	/**
	 ...
	 */
	function rook_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $ROOK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $ROOK_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $ROOK_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $ROOK_STORAGE[ $var_name ] ) ) {
				unset( $ROOK_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'rook_storage_inc' ) ) {
	/**
	 ...
	 */
	function rook_storage_inc( $var_name, $value = 1 ) {
		global $ROOK_STORAGE;
		if ( empty( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = 0;
		}
		$ROOK_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'rook_storage_concat' ) ) {
	/**
	 ...
	 */
	function rook_storage_concat( $var_name, $value ) {
		global $ROOK_STORAGE;
		if ( empty( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = '';
		}
		$ROOK_STORAGE[ $var_name ] .= $value;
	}
}

if ( ! function_exists( 'rook_storage_get_array' ) ) {
	/**
	 ...
	 */
	function rook_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $ROOK_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $ROOK_STORAGE[ $var_name ][ $key ] ) ? $ROOK_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'rook_storage_set_array' ) ) {
	/**
	 ...
	 */
	function rook_storage_set_array( $var_name, $key, $value ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $ROOK_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$ROOK_STORAGE[ $var_name ][] = $value;
			} else {
				$ROOK_STORAGE[ $var_name ][ $key ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'rook_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function rook_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $ROOK_STORAGE[ $var_name ][ $key ] ) ) {
			$ROOK_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $ROOK_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$ROOK_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$ROOK_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'rook_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function rook_storage_merge_array( $var_name, $key, $value ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ROOK_STORAGE[ $var_name ] = array_merge( $ROOK_STORAGE[ $var_name ], $value );
		} else {
			$ROOK_STORAGE[ $var_name ][ $key ] = array_merge( $ROOK_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'rook_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function rook_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			rook_array_insert_after( $ROOK_STORAGE[ $var_name ], $after, $key );
		} else {
			rook_array_insert_after( $ROOK_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'rook_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function rook_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			rook_array_insert_before( $ROOK_STORAGE[ $var_name ], $before, $key );
		} else {
			rook_array_insert_before( $ROOK_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'rook_storage_push_array' ) ) {
	/**
	 ...
	 */
	function rook_storage_push_array( $var_name, $key, $value ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $ROOK_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $ROOK_STORAGE[ $var_name ][ $key ] ) ) {
				$ROOK_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $ROOK_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'rook_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function rook_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $ROOK_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $ROOK_STORAGE[ $var_name ] ) && is_array( $ROOK_STORAGE[ $var_name ] ) && count( $ROOK_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $ROOK_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $ROOK_STORAGE[ $var_name ][ $key ] ) && is_array( $ROOK_STORAGE[ $var_name ][ $key ] ) && count( $ROOK_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $ROOK_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'rook_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function rook_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ROOK_STORAGE[ $var_name ][ $key ] ) ) {
			$ROOK_STORAGE[ $var_name ][ $key ] = 0;
		}
		$ROOK_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'rook_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function rook_storage_concat_array( $var_name, $key, $value ) {
		global $ROOK_STORAGE;
		if ( ! isset( $ROOK_STORAGE[ $var_name ] ) ) {
			$ROOK_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ROOK_STORAGE[ $var_name ][ $key ] ) ) {
			$ROOK_STORAGE[ $var_name ][ $key ] = '';
		}
		$ROOK_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'rook_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function rook_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $ROOK_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ROOK_STORAGE[ $var_name ] ) ? $ROOK_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ROOK_STORAGE[ $var_name ] ) ? $ROOK_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'rook_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function rook_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $ROOK_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $ROOK_STORAGE[ $var_name ]->$prop ) ? $ROOK_STORAGE[ $var_name ]->$prop : $default;
	}
}
