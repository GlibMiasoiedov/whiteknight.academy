...
if ( ! function_exists( 'rook_front_page_add_sidebars' ) ) {
	add_action( 'after_setup_theme', 'rook_front_page_add_sidebars', 3 );
	/**
	 * Add sidebars for front page sections to the global list of sidebars
	 * 
	 * @hooked 'after_setup_theme', 3
	 */
	function rook_front_page_add_sidebars() {
		if ( rook_get_theme_setting( 'allow_front_page_builder', true ) ) {
			add_filter( 'rook_filter_list_sidebars', 'rook_front_page_sidebars' );
		}
	}
}
...
// Theme init priorities:
// 3 - add/remove Theme Options elements
if ( ! function_exists( 'rook_front_page_setup3' ) ) {
	add_action( 'after_setup_theme', 'rook_front_page_setup3', 3 );
	/**
	 * Add options for the Front Page to the Theme Options and the Customizer
	 * 
	 * @hooked 'after_setup_theme', 3
	 */
	function rook_front_page_setup3() {
		if ( rook_get_theme_setting( 'allow_front_page_builder', true ) ) {
			rook_storage_set_array_before(
				'options', 'blog', apply_filters(
					'rook_filter_front_page_options', array(

						// 'Front Page Sections'
						'front_page'              => array(
							'title'      => esc_html__( 'Front Page Builder', 'rook' ),
							'desc'       => wp_kses_data( __( 'More fine tuning component display Front Page (view and menu position, presence and position of the sidebar, header and footer, etc.) you can produce in the section "Page Options" when editing a page, selected as Front Page', 'rook' ) ),
							'priority'   => 65,
						...
						'front_page_general_info' => array(
							'title' => esc_html__( 'General settings for Front Page Builder', 'rook' ),
							'desc'  => '',
							'type'  => 'info',
						),
						'front_page_enabled'      => array(
							'title' => esc_html__( 'Enable Front Page builder', 'rook' ),
							'desc'  => wp_kses_data( __( 'If Front Page Builder is off - native page content will be shown', 'rook' ) ),
							'std'   => 0,
							'type'  => 'switch',
						),
						'front_page_sections'     => array(
							'title'      => esc_html__( 'Sections order', 'rook' ),
							'desc'       => wp_kses_data( __( 'Drag and drop sections below to set up their order on the Front Page. You can also enable / disable any section.', 'rook' ) ),
							'dependency' => array(
						...
						'front_page_bg_image'     => array(
							...
							'desc'       => wp_kses_data( __( 'Select or upload background image for whole Front page', 'rook' ) ),
							'refresh'    => false,
							'dependency' => array(
								'front_page_enabled' => array( 1 ),
							),
							'std'        => ROOK_THEME_FREE ? rook_get_file_url( 'front-page/images/bg.jpg' ) : '',
							'type'       => 'image',
						),
					)
				)
...
if ( ! function_exists( 'rook_front_page_options_close' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_close', 20 );
	/**
	 * Close the panel with the Front Page options
	 * 
	 * @hooked 'rook_filter_front_page_options', 20
	 * 
	 * @param array $options  The Front Page options
	 * 
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_close( $options ) {
...
if ( ! function_exists( 'rook_front_page_options_title' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_title' );
	/**
	 * Add the section 'Title' to the Front Page options
	 * 
	 * @hooked 'rook_filter_front_page_options'
	 * 
	 * @param array $options  The Front Page options
	 * 
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_title( $options ) {
		...
		$options['front_page_sections']['options'] = array_merge(
			...
			array(
				'title' => esc_html__( 'Big title', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'front_page_title_slider_info'     => array(
					'title' => esc_html__( 'Slider', 'rook' ),
					'desc'  => '',
					'type'  => 'info',
				),
				'front_page_title_shortcode'       => array(
					'title'     => esc_html__( 'Slider Shortcode', 'rook' ),
					'desc'      => wp_kses_data( __( 'Paste a shortcode generated by any slider plugin. The slider will be used instead of the section title, description and buttons.', 'rook' ) ),
					'translate' => true,
				...
				'front_page_title_caption'         => array(
					...
					'std'        => wp_kses_data( __( 'Section with Big title', 'rook' ) ),
					'sanitize'   => 'wp_kses_post',
					'dependency' => array(
						'front_page_title_shortcode' => array( 'is_empty' ),
					),
				...
				'front_page_title_description'     => array(
					...
					'std'        => wp_kses_data( __( 'This text can be changed in the section "Title"', 'rook' ) ),
					'sanitize'   => 'wp_kses_post',
					'dependency' => array(
						'front_page_title_shortcode' => array( 'is_empty' ),
					),
				...
				'front_page_title_button1_link'    => array(
					'title'           => esc_html__( 'Button1 link', 'rook' ),
					'desc'            => '',
				...
				'front_page_title_button1_caption' => array(
					'title'      => esc_html__( 'Button1 caption', 'rook' ),
					'desc'       => '',
					'translate'  => true,
					'dependency' => array(
					...
					'refresh'    => false,
					'std'        => wp_kses_data( __( 'Customize Button 1', 'rook' ) ),
					'type'       => 'text',
				),
				'front_page_title_button2_link'    => array(
					'title'           => esc_html__( 'Button2 link', 'rook' ),
					'desc'            => '',
				...
				'front_page_title_button2_caption' => array(
					'title'      => esc_html__( 'Button2 caption', 'rook' ),
					'desc'       => '',
					'translate'  => true,
					'dependency' => array(
					...
					'refresh'    => false,
					'std'        => wp_kses_data( __( 'Customize Button 2', 'rook' ) ),
					'type'       => 'text',
				),
				...
				'front_page_title_scheme'          => array(
					...
					'std'     => ROOK_THEME_FREE ? 'dark' : 'inherit',
					'options' => array(),
					'refresh' => false,
					'type'    => 'radio',
				),
				'front_page_title_bg_image'        => array(
					...
					'refresh_wrapper' => true,
					'std'             => ROOK_THEME_FREE ? rook_get_file_url( 'front-page/images/bg-title.jpg' ) : '',
					'type'            => 'image',
				),
				...
				'front_page_title_bg_mask'         => array(
					...
					'desc'    => wp_kses_data( __( 'Use Background color as section mask with specified opacity. If 0 - mask is not being used', 'rook' ) ),
					'max'     => 1,
					'step'    => 0.1,
					'std'     => ROOK_THEME_FREE ? 0.5 : 1,
					'refresh' => false,
					'type'    => 'slider',
				),
				'front_page_title_anchor_info'     => array(
					'title' => esc_html__( 'Anchor', 'rook' ),
					'desc'  => wp_kses_data( __( 'You can select an icon and/or specify a text to create an anchor for this section to display it in the side menu (if selected in the section "Header - Menu").', 'rook' ) )
								. '<br>'
								. wp_kses_data( __( 'Attention! Anchors are available only if ThemeREX Addons plugin is installed and activated!', 'rook' ) ),
					'type'  => 'info',
				),
...
if ( ! function_exists( 'rook_front_page_options_features' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_features' );
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_features( $options ) {
		$options['front_page_sections']['std']    .= ( ! empty( $options['front_page_sections']['std'] ) ? '|' : '' ) . 'features=1';
		$options['front_page_sections']['options'] = array_merge(
			$options['front_page_sections']['options'],
			array(
				'features' => esc_html__( 'Features', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'sidebar-widgets-front_page_features_widgets' => array(
					'title'    => esc_html__( 'Features', 'rook' ),
					'desc'     => '',
					'priority' => 30,
					'type'     => 'section',
				),
				...
				'front_page_features_caption'      => array(
					...
					'std'       => wp_kses_data( __( 'Why our service is the best', 'rook' ) ),
					'priority'  => -80,
					'type'      => 'text',
				),
				'front_page_features_description'  => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Features"', 'rook' ) ),
					'priority'  => -70,
					'type'      => 'textarea',
				),
				'front_page_features_widgets_info' => array(
					'title'    => esc_html__( 'Widgets', 'rook' ),
					'desc'     => wp_kses_data( __( 'You can set up widgets for this section in "Appearance - Customize" or "Appearance - Widgets" tabs.', 'rook' ) )
								. '<br>'
								. wp_kses_data( __( 'Insert your preferred widget to display services here. You can also select any other widget, thus changing the purpose of this section', 'rook' ) ),
					'priority' => -60,
					'type'     => 'info',
				),
...
if ( ! function_exists( 'rook_front_page_options_about' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_about' );
	/**
		...
		$options                                   = array_merge(
			$options, array(
				...
				'front_page_about'              => array(
					'title'    => esc_html__( 'About Us', 'rook' ),
					'desc'     => '',
					'priority' => 40,
					'type'     => 'section',
				),
				...
				'front_page_about_caption'      => array(
					...
					'std'       => wp_kses_data( __( 'About Us', 'rook' ) ),
					'type'      => 'text',
				),
				'front_page_about_description'  => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "About"', 'rook' ) ),
					'type'      => 'textarea',
				),
				'front_page_about_content_info' => array(
					'title' => esc_html__( 'Content', 'rook' ),
					'desc'  => '',
					'type'  => 'info',
				),
				'front_page_about_content'      => array(
					'title'     => esc_html__( 'Content', 'rook' ),
					'desc'      => wp_kses_data( __( 'The arbitrary content of the current section.', 'rook' ) )
								. '<br>'
								. wp_kses_data(
									__( 'Attention! You can use %%CONTENT%% to insert instead the content of the page, selected as the Front Page in the menu "Settings - Reading" or in the "Customize - Static Front Page"', 'rook' )
								),
					'translate' => true,
...
if ( ! function_exists( 'rook_front_page_options_team' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_team' );
	/**
		...
		$options                                   = array_merge(
			$options, array(
				...
				'sidebar-widgets-front_page_team_widgets' => array(
					'title'    => esc_html__( 'Team members', 'rook' ),
					'desc'     => '',
					'priority' => 50,
					'type'     => 'section',
				),
				...
				'front_page_team_caption'                 => array(
					...
					'std'       => wp_kses_data( __( 'Meet our team', 'rook' ) ),
					'priority'  => -80,
					'type'      => 'text',
				),
				'front_page_team_description'             => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Team members"', 'rook' ) ),
					'priority'  => -70,
					'type'      => 'textarea',
				),
				'front_page_team_widgets_info'            => array(
					...
					'desc'     => wp_kses_data( __( 'You can set up widgets for this section in "Appearance - Customize" or "Appearance - Widgets" tabs.', 'rook' ) )
								. '<br>'
								. wp_kses_data( __( 'Insert your preferred widget to display team members here. You can also select any other widget, thus changing the purpose of this section', 'rook' ) ),
					'priority' => -60,
					'type'     => 'info',
				),
				...
				'front_page_team_bg_image'                => array(
					...
					'refresh_wrapper' => true,
					'std'             => ROOK_THEME_FREE ? rook_get_file_url( 'front-page/images/bg-team.jpg' ) : '',
					'type'            => 'image',
				),
...
if ( ! function_exists( 'rook_front_page_options_testimonials' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_testimonials' );
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_testimonials( $options ) {
		$options['front_page_sections']['std']    .= ( ! empty( $options['front_page_sections']['std'] ) ? '|' : '' ) . 'testimonials=1';
		$options['front_page_sections']['options'] = array_merge(
			$options['front_page_sections']['options'],
			array(
				'testimonials' => esc_html__( 'Testimonials', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'sidebar-widgets-front_page_testimonials_widgets' => array(
					'title'    => esc_html__( 'Testimonials', 'rook' ),
					'desc'     => '',
					'priority' => 60,
					'type'     => 'section',
				),
				...
				'front_page_testimonials_caption'      => array(
					...
					'std'       => wp_kses_data( __( 'What our clients say', 'rook' ) ),
					'priority'  => -80,
					'type'      => 'text',
				),
				'front_page_testimonials_description'  => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Testimonials"', 'rook' ) ),
					'priority'  => -70,
					'type'      => 'textarea',
				),
				'front_page_testimonials_widgets_info' => array(
					...
					'desc'     => wp_kses_data( __( 'You can set up widgets for this section in "Appearance - Customize" or "Appearance - Widgets" tabs.', 'rook' ) )
								. '<br>'
								. wp_kses_data( __( 'Insert your preferred widget to display testimonials here. You can also select any other widget, thus changing the purpose of this section', 'rook' ) ),
					'priority' => -60,
					'type'     => 'info',
				),
...
if ( ! function_exists( 'rook_front_page_options_blog' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_blog' );
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_blog( $options ) {
		$options['front_page_sections']['std']    .= ( ! empty( $options['front_page_sections']['std'] ) ? '|' : '' ) . 'blog=1';
		$options['front_page_sections']['options'] = array_merge(
			$options['front_page_sections']['options'],
			array(
				'blog' => esc_html__( 'Latest posts', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'sidebar-widgets-front_page_blog_widgets' => array(
					'title'    => esc_html__( 'Latest posts', 'rook' ),
					'desc'     => '',
					'priority' => 70,
					'type'     => 'section',
				),
				...
				'front_page_blog_caption'                 => array(
					...
					'std'       => wp_kses_data( __( 'Latest posts', 'rook' ) ),
					'priority'  => -80,
					'type'      => 'text',
				),
				'front_page_blog_description'             => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Latest posts"', 'rook' ) ),
					'priority'  => -70,
					'type'      => 'textarea',
				),
				'front_page_blog_widgets_info'            => array(
					...
					'desc'     => wp_kses_data( __( 'You can set up widgets for this section in "Appearance - Customize" or "Appearance - Widgets" tabs.', 'rook' ) )
								. '<br>'
								. wp_kses_data( __( 'Insert your preferred widget to display latest posts here. You can also select any other widget, thus changing the purpose of this section', 'rook' ) ),
					'priority' => -60,
					'type'     => 'info',
				),
...
if ( ! function_exists( 'rook_front_page_options_subscribe' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_subscribe' );
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_subscribe( $options ) {
		$options['front_page_sections']['std']    .= ( ! empty( $options['front_page_sections']['std'] ) ? '|' : '' ) . 'subscribe=1';
		$options['front_page_sections']['options'] = array_merge(
			$options['front_page_sections']['options'],
			array(
				'subscribe' => esc_html__( 'Subscribe', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'front_page_subscribe'                => array(
					'title'    => esc_html__( 'Subscribe', 'rook' ),
					'desc'     => '',
					'priority' => 80,
					'type'     => 'section',
				),
				...
				'front_page_subscribe_caption'        => array(
					...
					'std'       => wp_kses_data( __( 'Subscribe to our Newsletter', 'rook' ) ),
					'type'      => 'text',
				),
				'front_page_subscribe_description'    => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Subscribe"', 'rook' ) ),
					'type'      => 'textarea',
				),
				'front_page_subscribe_shortcode_info' => array(
					'title' => esc_html__( 'Shortcode', 'rook' ),
					'desc'  => '',
					'type'  => 'info',
				),
				'front_page_subscribe_shortcode'      => array(
					'title'     => esc_html__( 'Shortcode to insert Subscribe form', 'rook' ),
					'desc'      => wp_kses_data( __( 'Paste shortcode, generated with any subscribe plugin (for example, MailChimp)', 'rook' ) ),
					'translate' => true,
				...
				'front_page_subscribe_bg_image'       => array(
					...
					'refresh_wrapper' => true,
					'std'             => ROOK_THEME_FREE ? rook_get_file_url( 'front-page/images/bg-subscribe.jpg' ) : '',
					'type'            => 'image',
				),
...
if ( ! function_exists( 'rook_front_page_options_googlemap' ) ) {
	if ( ! ROOK_THEME_FREE ) {
		add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_googlemap' );
	}
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_googlemap( $options ) {
		$options['front_page_sections']['std']    .= ( ! empty( $options['front_page_sections']['std'] ) ? '|' : '' ) . 'googlemap=1';
		$options['front_page_sections']['options'] = array_merge(
			$options['front_page_sections']['options'],
			array(
				'googlemap' => esc_html__( 'Google map', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'sidebar-widgets-front_page_googlemap_widgets' => array(
					'title'    => esc_html__( 'Google map', 'rook' ),
					'desc'     => '',
					'priority' => 90,
					'type'     => 'section',
				),
				...
				'front_page_googlemap_layout'       => array(
					'title'           => esc_html__( 'Layout', 'rook' ),
					'desc'            => wp_kses_data( __( 'Select layout of this section', 'rook' ) ),
					'std'             => 'fullwidth',
					'options'         => array(
						'fullwidth' => esc_html__( 'Fullwidth', 'rook' ),
						'boxed'     => esc_html__( 'Boxed', 'rook' ),
						'columns'   => esc_html__( '2 columns', 'rook' ),
					),
				...
				'front_page_googlemap_caption'      => array(
					...
					'std'       => wp_kses_data( __( 'Google map', 'rook' ) ),
					'priority'  => -80,
					'type'      => 'text',
				),
				'front_page_googlemap_description'  => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Google map"', 'rook' ) ),
					'priority'  => -70,
					'type'      => 'textarea',
				),
				'front_page_googlemap_content'      => array(
					'title'     => esc_html__( 'Content', 'rook' ),
					'desc'      => wp_kses_data( __( 'Any text at the left side of the map', 'rook' ) ),
					'translate' => true,
				...
				'front_page_googlemap_widgets_info' => array(
					...
					'desc'     => wp_kses_data( __( 'You can set up widgets for this section in "Appearance - Customize" or "Appearance - Widgets" tabs.', 'rook' ) )
								. '<br>'
								. wp_kses_data( __( 'Insert your preferred widget to display the map with the location of your choice here. You can also select any other widget, thus changing the purpose of this section', 'rook' ) ),
					'priority' => -60,
					'type'     => 'info',
				),
				...
				'front_page_googlemap_bg_image'     => array(
					...
					'refresh'         => '.front_page_section_googlemap',
					'refresh_wrapper' => true,
					'std'             => ROOK_THEME_FREE ? rook_get_file_url( 'front-page/images/bg-googlemap.jpg' ) : '',
					'type'            => 'image',
				),
...
if ( ! function_exists( 'rook_front_page_options_contacts' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_contacts' );
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_contacts( $options ) {
		$options['front_page_sections']['std']    .= ( ! empty( $options['front_page_sections']['std'] ) ? '|' : '' ) . 'contacts=1';
		$options['front_page_sections']['options'] = array_merge(
			$options['front_page_sections']['options'],
			array(
				'contacts' => esc_html__( 'Contact Us', 'rook' ),
			)
		);
		$options                                   = array_merge(
			$options, array(
				...
				'sidebar-widgets-front_page_contacts_widgets' => array(
					'title'    => esc_html__( 'Contact Us', 'rook' ),
					'desc'     => '',
					'priority' => 100,
					'type'     => 'section',
				),
				...
				'front_page_contacts_caption'        => array(
					...
					'std'       => wp_kses_data( __( 'Contact Us', 'rook' ) ),
					'type'      => 'text',
				),
				'front_page_contacts_description'    => array(
					...
					'std'       => wp_kses_data( __( 'This text can be changed in the section "Contact Us"', 'rook' ) ),
					'type'      => 'textarea',
				),
				'front_page_contacts_content'        => array(
					'title'   => esc_html__( 'Content', 'rook' ),
					'desc'    => wp_kses_data( __( 'Any text at the left side of the form', 'rook' ) ),
					'refresh' => false, // To refresh part of the page: '.front_page_section_contacts .front_page_section_contacts_content',
					'std'     => wp_kses( __( '<h5><span class="icon-home-2"> </span>Find us at the office:</h5><p>500, Lorem Street,<br />Chicago, IL, 55030<br />Mon - Fri, 09:00 - 18:00</p><h5> <span class="icon-mobile-light"> </span>Give us a call:</h5><p>Michael Jordan<br />+40 (123) 456-78-90<br />Mon - Fri, 08:00 - 22:00</p>', 'rook' ), 'rook_kses_content' ),
					'type'    => 'text_editor',
				),
				...
				'front_page_contacts_shortcode'      => array(
					'title'     => esc_html__( 'Shortcode with contact form', 'rook' ),
					'desc'      => wp_kses_data( __( 'Paste shortcode, generated with any form plugin (for example, Contacts Form 7). You can also paste any other shortcodes, changing thus the purpose of this section', 'rook' ) ),
					'translate' => true,
...
if ( ! function_exists( 'rook_front_page_options_add_active_callback' ) ) {
	add_filter( 'rook_filter_front_page_options', 'rook_front_page_options_add_active_callback', 1000 );
	/**
	 ...
	 * @return array  The modified Front Page options
	 */
	function rook_front_page_options_add_active_callback( $options ) {
		foreach ( $options as $k => $v ) {
			if ( substr( $k, 0, 11 ) == 'front_page_' ) {
				$options[ $k ]['active_callback'] = 'rook_front_page_check';
			}
		}
		return $options;
	}
...
if ( ! function_exists( 'rook_front_page_check' ) ) {
	/**
	 ...
	 */
	function rook_front_page_check( $control = null ) {
...
if ( ! function_exists( 'rook_front_page_sidebars' ) ) {
	/**
	 * Add a Front Page specific items to the list of sidebars
	 * 
	 * @hooked 'rook_filter_list_sidebars'
	 * 
	 * @param array $list  The list of sidebars
	 * 
	 * @return array  The modified list of sidebars
	 */
	function rook_front_page_sidebars( $list = array() ) {
		$list['front_page_features_widgets']     = array(
			'name'               => wp_kses_data( __( 'Front Page section "Features"', 'rook' ) ),
			'description'        => wp_kses_data( __( 'Widgets to be shown only in the section "Features" on the front page', 'rook' ) ),
			'front_page_section' => true,
		);
		$list['front_page_team_widgets']         = array(
			'name'               => wp_kses_data( __( 'Front Page section "Team members"', 'rook' ) ),
			'description'        => wp_kses_data( __( 'Widgets to be shown only in the section "Team members" on the front page', 'rook' ) ),
			'front_page_section' => true,
		);
		$list['front_page_testimonials_widgets'] = array(
			'name'               => wp_kses_data( __( 'Front Page section "Testimonials"', 'rook' ) ),
			'description'        => wp_kses_data( __( 'Widgets to be shown only in the section "Testimonials" on the front page', 'rook' ) ),
			'front_page_section' => true,
		);
		$list['front_page_blog_widgets']         = array(
			'name'               => wp_kses_data( __( 'Front Page section "Latest Posts"', 'rook' ) ),
			'description'        => wp_kses_data( __( 'Widgets to be shown only in the section "Latest Posts" on the front page', 'rook' ) ),
			'front_page_section' => true,
		);
		if ( ! ROOK_THEME_FREE ) {
			$list['front_page_googlemap_widgets'] = array(
				'name'               => wp_kses_data( __( 'Front Page section "Google map"', 'rook' ) ),
				'description'        => wp_kses_data( __( 'Widgets to be shown only in the section "Google map" on the front page', 'rook' ) ),
				'front_page_section' => true,
			);
		}
		return $list;
...
if ( ! function_exists( 'rook_customizer_partial_refresh_section' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_section( $section ) {
		ob_start();
		get_template_part( apply_filters( 'rook_filter_get_template_part', "front-page/section-" . sanitize_file_name( $section ) ) );
		$output = ob_get_contents();
		ob_end_clean();
		return rook_customizer_partial_refresh_add_init_script( $output, $section );
	}
}


if ( ! function_exists( 'rook_customizer_partial_refresh_add_init_script' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_add_init_script( $output, $section ) {
		return sprintf(
			"%1$s<%2$s>
						setTimeout(function() {
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_title_button1_link' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_title_button1_link() {
		return rook_get_theme_option( 'front_page_title_button1_link' ) != ''
				? '<a href="' . esc_url( rook_get_theme_option( 'front_page_title_button1_link' ) ) . '" class="theme_button front_page_section_button front_page_section_title_button1">'
					. esc_html( rook_get_theme_option( 'front_page_title_button1_caption' ) )
					. '</a>'
				: '';
	}
}

if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_title_button2_link' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_title_button2_link() {
		return rook_get_theme_option( 'front_page_title_button2_link' ) != ''
				? '<a href="' . esc_url( rook_get_theme_option( 'front_page_title_button2_link' ) ) . '" class="theme_button color_style_link2 front_page_section_button front_page_section_title_button2">'
					. esc_html( rook_get_theme_option( 'front_page_title_button2_caption' ) )
					. '</a>'
				: '';
	}
}

if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_title_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_title_bg_image() {
		return rook_customizer_partial_refresh_section( 'title' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_about_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_about_bg_image() {
		return rook_customizer_partial_refresh_section( 'about' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_features_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_features_bg_image() {
		return rook_customizer_partial_refresh_section( 'features' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_team_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_team_bg_image() {
		return rook_customizer_partial_refresh_section( 'team' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_testimonials_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_testimonials_bg_image() {
		return rook_customizer_partial_refresh_section( 'testimonials' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_blog_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_blog_bg_image() {
		return rook_customizer_partial_refresh_section( 'blog' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_subscribe_shortcode' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_subscribe_shortcode() {
		$rook_sc = rook_get_theme_option( 'front_page_subscribe_shortcode' );
		return ! empty( $rook_sc ) ? do_shortcode( $rook_sc ) : '';
	}
}

if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_subscribe_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_subscribe_bg_image() {
		return rook_customizer_partial_refresh_section( 'subscribe' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_googlemap_layout' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_googlemap_layout() {
		return rook_customizer_partial_refresh_section( 'googlemap' );
	}
}

if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_googlemap_bg_image' ) ) {
	/**
	 * Callback to partial refresh a background image in the section 'Front Page - Google map'
	 * 
	 * @return string  The HTML output with the whole section 'Front Page - Google map'
	 */
	function rook_customizer_partial_refresh_front_page_googlemap_bg_image() {
		return rook_customizer_partial_refresh_section( 'googlemap' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_contacts_layout' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_contacts_layout() {
		return rook_customizer_partial_refresh_section( 'contacts' );
	}
}

if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_contacts_shortcode' ) ) {
	/**
	 * Callback to partial refresh a shortcode in the section 'Front Page - Contact Us'
	 * 
	 * @return string  The HTML output of the shortcode
	 */
	function rook_customizer_partial_refresh_front_page_contacts_shortcode() {
		$rook_sc = rook_get_theme_option( 'front_page_contacts_shortcode' );
		return ! empty( $rook_sc ) ? do_shortcode( $rook_sc ) : '';
	}
}

if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_contacts_bg_image' ) ) {
	/**
	 * Callback to partial refresh a background image in the section 'Front Page - Contact Us'
	 * 
	 * @return string  The HTML output with the whole section 'Front Page - Contact Us'
	 */
	function rook_customizer_partial_refresh_front_page_contacts_bg_image() {
		return rook_customizer_partial_refresh_section( 'contacts' );
	}
}
...
if ( ! function_exists( 'rook_customizer_partial_refresh_front_page_woocommerce_bg_image' ) ) {
	/**
	 ...
	 */
	function rook_customizer_partial_refresh_front_page_woocommerce_bg_image() {
		return rook_customizer_partial_refresh_section( 'woocommerce' );
	}
}
...
if ( !function_exists( 'rook_front_page_frontend_scripts' ) ) {
	add_action( 'wp_enqueue_scripts', 'rook_front_page_frontend_scripts', 1100 );
	/**
	 * Enqueue styles for frontend in the Front Page
	 * 
	 * @hooked 'wp_enqueue_scripts', 1100
	 */
	function rook_front_page_frontend_scripts() {
		if ( rook_get_theme_setting( 'allow_front_page_builder', true )
			&& is_front_page()
			&& ! is_home()
			&& get_option( 'show_on_front' ) == 'page'
			&& rook_is_on( rook_get_theme_option( 'front_page_enabled', false ) )
		) {
			$rook_url = rook_get_file_url( 'front-page/front-page.css' );
			if ( '' != $rook_url ) {
				wp_enqueue_style( 'rook-front-page',  $rook_url, array(), null );
			}
			$rook_url = rook_get_file_url( 'front-page/front-page-responsive.css' );
			if ( '' != $rook_url ) {
				wp_enqueue_style( 'rook-front-page-responsive',  $rook_url, array(), null, rook_media_for_load_css_responsive( 'front-page' ) );
			}
		}
	}
}
