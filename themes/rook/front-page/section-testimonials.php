<div class="front_page_section front_page_section_testimonials<?php
	$rook_scheme = rook_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $rook_scheme ) && ! rook_is_inherit( $rook_scheme ) ) {
		echo ' scheme_' . esc_attr( $rook_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( rook_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( rook_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$rook_css      = '';
		$rook_bg_image = rook_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$rook_anchor_icon = rook_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$rook_anchor_text = rook_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( rook_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$rook_css      = '';
			$rook_bg_mask  = rook_get_theme_option( 'front_page_testimonials_bg_mask' );
			$rook_bg_color_type = rook_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $rook_bg_color_type ) {
				$rook_bg_color = rook_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$rook_caption = rook_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $rook_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $rook_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $rook_caption, 'rook_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$rook_description = rook_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $rook_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $rook_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $rook_description ), 'rook_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! rook_exists_trx_addons() ) {
						...
						rook_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
