...
if ( ! defined( 'ROOK_THEME_FREE' ) ) {
	define( 'ROOK_THEME_FREE', false );
}
if ( ! defined( 'ROOK_THEME_FREE_WP' ) ) {
	define( 'ROOK_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'ROOK_THEME_ALLOW_UPDATE' ) ) {
	define( 'ROOK_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'ROOK_ALLOW_SKINS' ) ) {
	define( 'ROOK_ALLOW_SKINS', true );
}
if ( ! defined( 'ROOK_DEFAULT_SKIN' ) ) {
	define( 'ROOK_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'ROOK_REMEMBER_SKIN' ) ) {
	define( 'ROOK_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'rook_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function rook_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = rook_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'rook_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'rook_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function rook_trx_updater_get_theme_info( $info ) {
		$engine = rook_storage_get( 'engine' );
...
if ( ! function_exists( 'rook_original_theme_slug' ) ) {
	add_filter( 'rook_filter_original_theme_slug', 'rook_original_theme_slug' );
	/**
...
if ( ! function_exists( 'rook_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'rook_importer_set_options', 9 );
	/**
	 ...
	 */
	function rook_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( rook_get_protocol() . ':' . rook_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( rook_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'rook_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'rook_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function rook_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( rook_get_protocol() . ':' . rook_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'rook_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'rook_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function rook_customizer_theme_setup1() {
		rook_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'rook_filter_allow_fullscreen' return false)
