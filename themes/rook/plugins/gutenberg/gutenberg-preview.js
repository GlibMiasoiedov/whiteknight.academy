...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			rook_create_observer( 'rook-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				rook_gutenberg_editor_init();
			} );
		} else {
		 	rook_gutenberg_editor_init();
		}
	...
	function rook_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'rook_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'rook_inited' ) ) {
	...
	// Init on page load
	function rook_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = rook_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + ROOK_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + ROOK_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in ROOK_STORAGE['override_classes'] ) {
				$obj.addClass( ROOK_STORAGE['override_classes'][i].replace( '%s', ROOK_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( rook_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && ROOK_STORAGE['sidebar_position'] != 'hide' ) {
				if ( rook_apply_filters( 'rook_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'rook_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( rook_apply_filters( 'rook_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				rook_remove_observer( 'rook-check-editor-styles-wrapper' );
				rook_create_observer( 'rook-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'rook_inited' );
	}
} );
