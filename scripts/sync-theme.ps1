# Sync theme files from WordPress server to local

. "$PSScriptRoot\..\.agent\config.ps1"

$localThemesDir = "$PSScriptRoot\..\themes"
...
Write-Host "Syncing rook-child theme from server..." -ForegroundColor Cyan
Download-FtpDirectory -FtpPath "/wp-content/themes/rook-child/" -LocalPath "$localThemesDir\rook-child"
